...
class ExpertService {
  ...
  async getExpertByUserId(userId) {
    try {
      ...
      logger.error('Get expert by user ID error:', error);
      throw error;
    }
  }
  ...
  async getExpertAvailability(expertId, date) {
    try {
      ...
      if (date) {
        ...
        availability = await executeQuery(
          'SELECT * FROM expert_availability WHERE expert_id = ? AND day_of_week = ? AND is_available = TRUE',
          [expertId, dayOfWeek]
        );
      } else {
      ...
      logger.error('Get expert availability error:', error);
      throw error;
    }
  }
  ...
  async createExpert(expertData) {
    try {
      const {
        id, userId, credentials, bio, specialties = [], availability = []
      } = expertData;

      // Check if user exists and is not already an expert
      ...
      const existingExperts = await executeQuery(
        'SELECT id FROM experts WHERE user_id = ?',
        [userId]
      );

      if (existingExperts.length > 0) {
        throw new Error('User is already an expert');
      }
      ...
      return await this.getExpertById(id);
    } catch (error) {
  ...
  async updateExpert(id, updateData) {
    try {
      ...
      if (updateData.specialties) {
        await this.updateExpertSpecialties(id, updateData.specialties);
      }
  ...
  async updateExpertSpecialties(expertId, specialties) {
    try {
      const queries = [
        // Remove existing specialties
        {
          query: 'DELETE FROM expert_specialties WHERE expert_id = ?',
          params: [expertId]
        }
      ];
      ...
      specialties.forEach(specialty => {
        queries.push({
          query: 'INSERT INTO expert_specialties (id, expert_id, specialty) VALUES (?, ?, ?)',
          params: [uuidv4(), expertId, specialty]
        });
      });

      await executeTransaction(queries);
    } catch (error) {
      logger.error('Update expert specialties error:', error);
      throw error;
    }
  }

  // Update expert availability
  async updateExpertAvailability(expertId, availability) {
    try {
      const queries = [
        // Remove existing availability
        {
          query: 'DELETE FROM expert_availability WHERE expert_id = ?',
          params: [expertId]
        }
      ];
      ...
      availability.forEach(slot => {
        queries.push({
          query: `INSERT INTO expert_availability 
                  (id, expert_id, day_of_week, start_time, end_time, is_available) 
                  VALUES (?, ?, ?, ?, ?, ?)`,
          params: [uuidv4(), expertId, slot.dayOfWeek, slot.startTime, slot.endTime, slot.isAvailable]
        });
      });
      ...
      logger.error('Update expert availability error:', error);
      throw error;
    }
  }

  // Update expert by user ID
  async updateExpertByUserId(userId, updateData) {
    try {
      // Get expert ID
      const experts = await executeQuery(
        'SELECT id FROM experts WHERE user_id = ?',
      ...
      return await this.updateExpert(experts[0].id, updateData);
    } catch (error) {
      logger.error('Update expert by user ID error:', error);
      throw error;
    }
  }

  // Update expert availability by user ID
  async updateExpertAvailabilityByUserId(userId, availability) {
    try {
      // Get expert ID
      const experts = await executeQuery(
        'SELECT id FROM experts WHERE user_id = ?',
      ...
      return await this.updateExpertAvailability(experts[0].id, availability);
    } catch (error) {
      logger.error('Update expert availability by user ID error:', error);
      throw error;
    }
  }
